...
const ManageTeams = () => {
  ...
  const { teams = [], loading, error, successMessage } = useSelector((state) => state.teams || {});
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try { await dispatch(deleteTeam(deleteId)); setShowDeleteModal(false); setDeleteId(null); }
    catch { toast.error("Failed to delete team."); }
  };

  const handleSaveChanges = async (updatedTeam) => {
    if (!updatedTeam) return;
    try { await dispatch(updateTeam(updatedTeam)); setShowEditModal(false); setSelectedTeam(null); }
  ...
  const filteredTeams = teams.filter(team => {
    ...
    const matchesStatus =
      statusFilter === "" ? true
      : statusFilter === "active" ? team.status === true
      : team.status === false;
  ...
  return (
    <div className="container-fluid position-relative bg-white d-flex p-0">
      ...
      <div className="content flex-grow-1" style={{ marginLeft: windowWidth >= 992 ? (isSidebarOpen ? 259 : 95) : (isSidebarOpen ? 220 : 0), transition: "margin-left 0.3s ease" }}>
        ...
        <div className="container-fluid px-4 pt-3">
          ...
          <div className="row mb-4">
            <div className="bg-white p-3 rounded shadow-sm card-header d-flex justify-content-between align-items-center">
              ...
              <Link to="/add-team" className="btn btn-new-lead"><i className="bi bi-plus-circle me-1"></i> Add Team</Link>
            </div>
          </div>
            ...
            <div className="bg-white p-3 rounded shadow-sm card-header">
              <div className="row g-2 align-items-center">
                <div className="col-md-4">
                  <input type="text" className="form-control border-0" placeholder="Search by Name" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
              ...
              <div className="table-responsive">
                ...
                <table className="table align-middle table-striped table-hover">
                  <thead className="table-light">
                    <tr>
                      <th>S.no</th><th>Name</th><th>Gender</th><th>Email</th><th>Role</th><th>Address</th><th>Status</th><th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredTeams.map((team, index) => (
                      <tr key={team.id}>
                        ...
                        <td>{team.gender || "-"}</td>
                        <td>{team.email}</td>
                        <td>{team.userRole === 1 ? "Admin" : team.userRole === 0 ? "User" : "Editor"}</td>
                        <td>{team.address || "-"}</td>
                        <td>
          ...
          {/* Modals */}
          <EditTeamModal showEditModal={showEditModal} setShowEditModal={setShowEditModal} selectedTeam={selectedTeam} handleSaveChanges={handleSaveChanges} />
          <ViewTeamModal showViewModal={showViewModal} setShowViewModal={setShowViewModal} selectedTeam={selectedTeam} baseUrl={BASE_URL} />
          <DeleteConfirmationModal show={showDeleteModal} handleClose={() => setShowDeleteModal(false)} handleConfirm={confirmDelete} />
        </div>
